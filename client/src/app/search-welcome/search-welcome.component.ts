...
export class SearchWelcomeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.slt_pickpdate = this.formatDate(today);
    this.slt_droppdate = this.formatDate(dropDate);
  }
  ...
  public prepareSearch() {
    if (this.slt_pickplace) {
      this.slt_pickplace = this.slt_pickplace.trim();
    }

    if (!this.slt_dropplace || this.slt_dropplace.trim() === '') {
      this.slt_dropplace = this.slt_pickplace;
    } else {
      this.slt_dropplace = this.slt_dropplace.trim();
    }
  }
